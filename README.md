...
| 태그 명칭      | 설명                                              | 분류    | 비고                                     |
| :------------- | :------------------------------------------------ | :------ | :--------------------------------------- |
| `[Origin]`     | 원본 파일명을 입력                                | 기본    | 목록에서 사라지기 전까지 변경되지 않음   |
| `[Name]`       | 현재 파일명을 입력                                | 기본    | 변경 적용사항이 반영됨                   |
| `[OnlyNumber]` | 파일명에서 숫자만 남김                            | 기본    | OnlyLetter, ToUpper/Lower와 베타적 관계  |
| `[OnlyLetter]` | 파일명에서 숫자, 특수문제를 제거함                | 기본    | OnlyNumber와 베타적 관계                 |
| `[ToUpper]`    | 파일명의 모든 알파벳을 대문자로 변환              | 기본    | OnlyLetter, ToLower와 베타적 관계        |
| `[ToLower]`    | 파일명의 모든 알파벳을 소문자로 변환              | 기본    | OnlyLetter, ToUpper와 베타적 관계        |
| `[Number]`     | 시작 값에서부터 순차적으로 증가하는 수를 입력     | 옵션    | 시작값, 자릿수 설정 가능                 |
| `[AtoZ]`       | 시작 알파벳에서 A-Z 순서로 증가하는 알파벳을 입력 | 옵션    | 시작값, 자릿수, 대소문자 설정 가능       |
| `[Name.trim]`  | 파일명의 특정 범위를 잘라내거나 남김              | 옵션    | 시작 위치, 범위, 자르기/남기기 설정 가능 |
| `[Today]`      | 오늘 날짜를 형식(YY, MM, DD 등)에 맞춰 입력       | 옵션    | 자유로운 조합 가능, 구분자 자유 설정     |
| `[Time.now]`   | 현재 시간을 형식(HH, MM, SS 등)에 맞춰 입력       | 옵션    | 자유로운 조합 가능, 구분자 자유 설정     |
